// memanggil si express
const express = require(`express`)

// membuat object `app`
const app = express()

// minta izin utk membaca data yg dikirimkan melalui form
app.use(express.urlencoded({ extended: true }))

// panggil authorization
const authorization = require(`../middleware/authorization`)

// memanggil controller admin
const adminController = require(`../controllers/admin.controller`)

// defind row untuk akses data admin
app.get(`/`,authorization.cekUser,adminController.showDataadmin)

// defind row untuk menampilkan form admin
app.get(`/add`,authorization.cekUser,adminController.showTambahadmin)

// defind routes untuk memproses tambah data admin
app.post(`/add`,authorization.cekUser,adminController.prosesTambahData)

// define route utk nampilin form admin dg data yg akan di ubah
app.get(`/edit/:id`,authorization.cekUser,adminController.showEditadmin)

/**define route untuk memproses perubahan data */
app.post(`/edit/:id`,authorization.cekUser,adminController.prosesUbahData)

// define route untuk menghapus data
app.get(`/delete/:id`,authorization.cekUser,adminController.prosesHapusData)

// export object app
module.exports = app